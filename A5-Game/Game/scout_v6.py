...
class MyPlayer(Player):
	...
	def set_mines(self, status):
		# only mine if we can afford it comfortably
		if status.gold < 30:
			return []
		...
		for x in range(self.width):
			for y in range(self.height):
				...
				opp_dist = max(abs(x - me[0]), abs(y - me[1]))
				if opp_dist > 2:
					continue

				# drop a distance-1 mine on the tile between us, if valid
				dx = (x - me[0]) // max(1, abs(x - me[0])) if x != me[0] else 0
				dy = (y - me[1]) // max(1, abs(y - me[1])) if y != me[1] else 0
				target = (me[0] + dx, me[1] + dy)

				if not (0 <= target[0] < self.width and 0 <= target[1] < self.height):
					continue
				t = self.known_map[target]
				if t.status != TileStatus.Empty or t.obj is not None:
					continue

				mines.append(target)
				break  # for now max one mine per turn 
