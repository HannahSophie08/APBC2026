...
class MyPlayer(Player):
	...
	def find_frontier_path(self, start):
		# bfs over ONLY tiles we have already seen are safe, stop at the nearest frontier.
		# dead ends fix themselves: a fully explored sackgasse holds no frontier, so bfs
		# routes us back out toward real openings instead of pingponging at the entrance
		queue = deque()
		queue.append((start, []))
		visited = {start}
		...
		while queue:
			...
			# reachable opening found, go there
	...
	def explore_step(self, start):
		path = self.find_frontier_path(start)
		if not path:
			return []
		# one step per round keeps cost at 1 and lets us re-plan with fresh vision each turn.
		# bump the slice (e.g. path[:3]) if traversal feels too slow, but k steps cost k(k+1)/2
		return [path[0]]
		...
		if status.goldPots:
			...
			if path:
				...
				if rounds_needed > status.goldPotRemainingRounds:
					...
					# explore reveals map for the next pot; returns [] -> we bank.
					return self.explore_step(start)
